...
class FollowRequestViewSet(viewsets.ViewSet):
    ...
    def create(self, request, user_id=None):
        requester_id = request.user.id  # O usuário logado é o solicitante (requester)
        requested_id = user_id  # O ID do usuário a ser seguido é passado na URL
        ...
        try:
            ...
            # Cria a solicitação de seguimento
    ...
    def handle_request(self, request, pk=None, action=None):
        try:
            # Recupera a solicitação de seguimento
            follow_request = FollowRequestService.retrieve_follow_request(pk)
            
            # Verifica se o usuário autenticado é o "requested" (destinatário)
            ...
            # Executa a ação (aceitar ou rejeitar)
            result = FollowRequestService.handle_follow_request(follow_request.id, action)  # Mude aqui para `result`
