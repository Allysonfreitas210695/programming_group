...
class UserFollowingViewSet(viewsets.ViewSet):
    ...
    # Listar os seguidores do usuário logado
    ...
    def list_non_friends(self, request):
        # Obtenha os usuários não seguidores
        non_friends = UserFollowingService.list_non_friends(request.user.id)
        
        # Use o UserSerializer para serializar os usuários não seguidores
        serializer = UserSerializer(non_friends, many=True)
        
        # Formatar a resposta para incluir apenas id e name
    ...
    def remove_follower(self, request, follower_id=None):
        try:
            # Passa o ID do usuário que está fazendo a requisição e o ID do seguidor a ser removido
